...
public class UserResource extends BaseResource {
    ...
    public Response registerRequest(
        ...
        // 检查是否有未审核的请求
        UserRegisterRequestDao reqDao = new UserRegisterRequestDao();
        List<UserRegisterRequest> pending = reqDao.findAllPending();
        for (UserRegisterRequest req : pending) {
            if (req.getUsername().equals(username)) {
                throw new ClientException("AlreadyPendingRequest", "A pending request already exists");
            }
        }
        ...
        req.setEmail(email);
        req.setStorageQuota(storageQuota != null ? storageQuota : 10000L);
        reqDao.create(req);
    ...
    public Response getRegisterRequests() {
        checkBaseFunction(BaseFunction.ADMIN);
        UserRegisterRequestDao reqDao = new UserRegisterRequestDao();
        List<UserRegisterRequest> list = reqDao.findAllPending();
        JsonArrayBuilder arr = Json.createArrayBuilder();
        for (UserRegisterRequest req : list) {
            arr.add(Json.createObjectBuilder()
                    .add("id", req.getId())
                    .add("username", req.getUsername())
                    .add("email", req.getEmail() != null ? req.getEmail() : "")
                    .add("storage_quota", req.getStorageQuota())
    ...
    public Response approveRegisterRequest(@PathParam("id") String id) {
        checkBaseFunction(BaseFunction.ADMIN);
        UserRegisterRequestDao reqDao = new UserRegisterRequestDao();
        UserRegisterRequest req = reqDao.getById(id);
        ...
        user.setRoleId(Constants.DEFAULT_USER_ROLE);
        user.setStorageQuota(req.getStorageQuota() * 1024 * 1024); // 转为字节
        user.setOnboarding(true);
        user.setDisableDate(null);
        UserDao userDao = new UserDao();
        try {
            ...
            throw new ServerException("CreateUserError", "Failed to create user", e);
        }

        // 更新请求状态
        req.setStatus(1);
        req.setOperatedDate(new Date());
        reqDao.update(req);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response rejectRegisterRequest(@PathParam("id") String id, @FormParam("reason") String reason) {
        checkBaseFunction(BaseFunction.ADMIN);
        UserRegisterRequestDao reqDao = new UserRegisterRequestDao();
        UserRegisterRequest req = reqDao.getById(id);
        if (req == null || !"pending".equals(req.getStatus())) {
